...
class BottlePositionAdjuster:
    def __init__(self):
        ...
        # 控制参数
        self.kp_distance = 0.003       # 距离调整比例系数
        self.kp_rotation = 0.005       # 旋转调整比例系数
        ...
        self.max_adjust_time = 45.0     # 最大调整时间(秒)
        ...
        self.current_linear = 0.0  # 线速度
        self.current_angular = 0.0 # 角速度
    ...
    def detect_bottle(self, frame):
        ...
        for result in results:
            ...
            for box in boxes:
                conf = box.conf.item()
                if conf > max_conf and conf > 0.5:  # 置信度阈值
                    max_conf = conf
    ...
    def calculate_adjustment(self, dx, size_error, rotation_error):
        ...
        if dx is not None and size_error is not None and rotation_error is not None:
            ...
            if abs(dx) < 50:
                angular_z *= 0.5
                linear_x = -self.kp_size * size_error * 1.5 
            else:
                linear_x *= 0.3
    ...
    def visualize_detection(self, frame, box, label, conf, dx=None, size_error=None, rotation_error=None):
        ...
        # 添加目标大小信息
        cv2.putText(frame, f"目标高度: {self.target_height}px", (10, frame.shape[0]-40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        ...
        if box:
            ...
            # 显示检测框高度
            cv2.putText(frame, f"当前高度: {height}px", (x1, y1 - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
            ...
            # 绘制从理想中心到当前中心的连线
            cv2.line(frame, (int(self.target_center_x), int(self.target_center_y)), (center_x_box, center_y_box), (0, 255, 255), 2)
        ...
        if dx is not None and size_error is not None:
            ...
            if self.adjustment_complete:
                comp_text = "调整完成!"
                cv2.putText(frame, comp_text, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        else:
            cv2.putText(frame, "寻找目标...", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        
        # 警告信息
        if self.subscriber_warning_shown:
            warn_text = "警告: /cmd_vel无订阅者!"
            cv2.putText(frame, warn_text, (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            warn_text2 = "请手动启动底盘节点!"
            cv2.putText(frame, warn_text2, (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        
        # 时间戳
        now = datetime.datetime.now()
        time_str = now.strftime("%Y-%m-%d %H:%M:%S")
        cv2.putText(frame, time_str, (10, frame.shape[0]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
        
        # 目标位置信息
        target_text = f"目标位置: ({self.target_center_x:.0f}, {self.target_center_y:.0f})"
        cv2.putText(frame, target_text, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 1)
