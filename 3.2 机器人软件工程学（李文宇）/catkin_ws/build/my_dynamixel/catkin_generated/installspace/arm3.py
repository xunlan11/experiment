...
class MultiJointControl:
    def __init__(self):
        ...
        # 设置10秒后关闭的定时器
        rospy.Timer(rospy.Duration(10), self.shutdown_timer_callback, oneshot=True)
    ...
    def shutdown_timer_callback(self, event):
        """10秒后触发关闭节点"""
        rospy.loginfo("10 seconds elapsed, shutting down...")
    ...
    def move_to_first_positions(self):
        """每个关节移动到目标位置"""
        target_positions = [-1.5, -1.5, 0.5, -1, 0]  # 关节1-5的目标位置(弧度)
